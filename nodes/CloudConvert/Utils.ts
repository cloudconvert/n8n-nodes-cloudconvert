...
export async function createJob(this: IExecuteFunctions, tasks: CreateTasksPayload) {
...
export async function uploadInputFile(
	...
	if (this.getNodeParameter('inputBinaryData', itemIndex)) {
		...
		let buffer = await this.helpers.getBinaryDataBuffer(itemIndex, binaryPropertyName);
...
export function getJobUploadTask(job: Job, index: number = 0): Task | null {
