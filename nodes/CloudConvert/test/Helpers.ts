...
const loadKnownNodes = (): Record<string, LoadingDetails> => {
	knownNodes = {
		"manualTrigger": {
			...
			"sourcePath": "node_modules/n8n-nodes-base/dist/nodes/ManualTrigger/ManualTrigger.node.js"
		},
		"cloudConvert": {
			"className": "CloudConvert",
			"sourcePath": "dist/nodes/CloudConvert/CloudConvert.node.js"
		}
	};
	return knownNodes;
};
...
export const getWorkflowFilenames = (dirname: string) => {
	...
	filenames.forEach((file) => {
		if (file.includes('.json')) {
			workflows.push(path.join(dirname, file));
		}
	});
